...
- `pnpm -F @loom-js/core type-check-tests` — `tsc -p tests/tsconfig.json`. wtr's esbuild only _transpiles_, so this script is the only thing that type-checks `tests/**`; run it alongside `type-check`.
...
- **Prettier config** (`.prettierrc`): 4-space tabs, single quotes, semicolons, no trailing commas, plus `prettier-plugin-sort-imports` (Sander Ronde's — **not** `@trivago/prettier-plugin-sort-imports`; the two share a short name but have different options) giving NPM-imports-then-local-imports ordering, a blank line between groups, alphabetical within each. Run `pnpm format` / `pnpm format:check`; the check also runs in CI (`format-check.yml`). Formatting matters for imports — let prettier handle them.
    ...
    - Four dev tools embed the legacy TypeScript compiler API that `typescript@7` removed (`prettier-plugin-sort-imports`, `rollup-plugin-dts`, `@rollup/plugin-typescript`, `ts-node`). Each gets its own nested `typescript@6` via the `readPackage` hook in `.pnpmfile.cjs` — overrides and `packageExtensions` cannot do this, because pnpm resolves their `typescript` peer from the workspace context (which is 7.x). Drop a tool's entry when it ships TS 7 support — see `openspec/changes/archive/2026-08-02-fix-prettier-import-sorting/`. Corollary: never pass the workspace's `typescript` into these tools (e.g. the `typescript:` option of `@rollup/plugin-typescript` in `rollup.config.ts`) — they must use their own. A fifth case, `@vercel/node` (compiles `services/api/*.ts` at deploy time), resolves TypeScript from the `services` workspace directly rather than via a peer dep, so `services/package.json` pins its own `typescript: ^6.0.2` — keep that pin until Vercel's builder supports TS 7.
...
- **`.npmrc` hoist patterns**: `@aws-sdk/*` and `@smithy/*` are public-hoisted for `@vercel/node`'s deploy-time type-check, which resolves with `preserveSymlinks` and cannot traverse pnpm's symlinked store — without the hoist, `S3Client`'s base class collapses and inherited methods (`send`) vanish as TS2339 in the services build logs. Reproduce with `vercel build` or `tsc --preserveSymlinks` in `services/`.
