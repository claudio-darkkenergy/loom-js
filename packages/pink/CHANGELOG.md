...
- cc42b73: Pink is converted to element syntax end to end, and the `@loom-js/tags` wrapper layer it sat on is retired — `@loom-js/tags` is no longer a dependency, and `packages/tags` is deleted from the repository.
...
- 9cc3f3a: `PinkGridHeader` now takes its columns as named slots instead of the `gridCol1`–`gridCol4` object props:
...
- e309b37: `PinkGridHeader` forwards its header props to `Header` via the new spread form (`...${{ attrs, id, on, style }}`) instead of enumerating them. No behavior change — the rendered DOM is byte-identical — but this release requires a `@loom-js/core` with spread-prop support.
