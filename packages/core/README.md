...
The boundary: internal reactivity — the component's own effects and binds — re-renders content without re-running the render function, so the activity persists. A _parent_ re-rendering this component re-runs the render function and recreates the activity, resetting it to its initial value. State that must survive parent-driven re-renders (or be shared between instances) belongs at module scope.
