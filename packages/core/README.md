...
- `update(next: V)` - Commits a value; call it as many times as needed. It deliberately shadows the activity's own `update`: inside a transform, updating _is_ committing — dispatching the activity from within its own transform would loop infinitely.
