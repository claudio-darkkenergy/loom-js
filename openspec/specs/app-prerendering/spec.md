# app-prerendering Specification

## Purpose

Defines the loom app's SSG contract: production builds emit fully rendered static HTML for the home route and every published docs topic, discovered from the Contentful page listing, rendered through the same code path the client runs, serialized only once settled, and each carrying its dehydrated resource state for the client to prime from.

Established by the `server-first-loom-app` change (2026-09-15). On-demand regeneration (ISR) remains a future evolution via the reusable per-route render entry.
