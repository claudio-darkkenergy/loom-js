# activity-transform-concurrency Specification

## Purpose

Defines the dispatch semantics for activities with asynchronous transforms — how overlapping `update()` calls relate: latest-dispatch-wins supersession by default (with a per-run `AbortSignal` for cooperative cancellation), opt-in `'ordered'` (parallel execution, commits applied in dispatch order) and `'serial'` (queued execution, predecessor-visible `value`) modes, `timeout` retirement of hung runs, settlement-signal interaction, the read-only `input` contract, and the sync-path invariants every mode preserves.

Established by the `activity-transform-concurrency` change (2026-09-10). Commits a transform makes to other activities (fan-out) sit outside the run's gated `update`; the run's `signal` is the consumer-side guard for those.
