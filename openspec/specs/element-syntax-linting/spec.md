...
Defines the project's obligations for static enforcement of loom's element syntax via `@loom-js/eslint-plugin`. The `add-lint-plugin` change (2026-08-16) established the capability: a `prefer-element-syntax` rule that reports direct component-call interpolations in child positions of loom templates, a `no-dollar-props-on-component-tags` rule that reports `$`-prefixed attributes on component tags, an eslint 9 flat-config plugin surface, and a lint-only GritQL mirror for Biome users.
