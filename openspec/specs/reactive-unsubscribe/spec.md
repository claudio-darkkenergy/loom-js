...
Defines the disposal contract for loom's reactive subscription layer: `reactiveEffect` returns an unsubscriber that permanently stops the effect and removes it from every dependency set it occupied, `activity(...).watch` surfaces that unsubscriber to callers, and `watchRoute` forwards it per its documented contract. Disposal is idempotent and safe to invoke during a trigger pass, and a disposed effect can never be re-tracked back in.

This capability covers caller-managed subscriptions (`reactiveEffect`, `watch`, `watchRoute`) in `@loom-js/core`. Render subscriptions created by `activity.effect` remain context-managed and are the subject of the future automatic unmount-driven teardown capability.
