...
Defines the project's obligations for multi-region content composition — named slots. A component can receive multiple labelled content regions: `slot="name"`-labelled top-level children of a component element are grouped by label and delivered as per-region `ContextFunction`s on `props.slots`, with the unlabelled remainder forming ordinary `children`. Distribution is loom's own in the light DOM and the platform's inside shadow roots; both composition forms (element syntax and the functional call it compiles to) express the same regions, and labels outside the accepted grammar throw at transform time rather than silently mis-distributing.

This capability extends `template-component-syntax` (the children-region compilation) and stays deliberately short of native `<slot>` semantics: no fallback content, no `slotchange`, no `assignedNodes` emulation.
