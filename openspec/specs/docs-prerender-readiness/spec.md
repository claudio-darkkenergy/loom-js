# docs-prerender-readiness Specification

## Purpose

Defines the server-first readiness contract for the docs app: components render under `renderToString` against an injected (linkedom) window with browser-only APIs confined to `onMounted` and event handlers, docs and home content fetches route through core's keyed `resource()` cache so the settlement signal tracks them and `dehydrate()` can capture their values, and published topic slugs are permanent — under prerendering they become physical static paths, so renames ship explicit redirects. The server-first pipeline itself (SSG, hydration boot, edge caching) belongs to the sibling `server-first-loom-app` change; this capability guarantees the docs app blocks none of it.
