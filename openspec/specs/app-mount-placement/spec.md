...
Defines how the boot entries position the app node relative to the mount root's existing children. The position is a self-documenting placement mode — `'replace' | 'append' | 'prepend'`, defaulting to `'replace'` — carried by `init`'s public contract (`AppInitProps.placement`) and by the internal mount seam (`lib/mount.ts`), replacing the former three-valued `append?: boolean | null` prop. Introduced by the `core-api-follow-ups` change (2026-08-19).
