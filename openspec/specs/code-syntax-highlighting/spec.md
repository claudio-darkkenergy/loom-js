...
Defines how a code panel turns source text into themed, classed tokens. Introduced by the `code-syntax-highlighting` change (2026-08-29): `@loom-js/highlight` owns the tokenizer (Prism), the canonical token-kind vocabulary, and grammar loading behind a settlement-tracked `lazyImport`; `@loom-js/pink`'s `PinkCodePanel.Content` renders any tokenizer passed through its structural `tokenize`/`language` contract and themes the kinds via `--p-code-token-<kind>` CSS variables (Ayu Dark preset, brand-tinted; AA-safe light counterpart). The docs app forwards its `// @lang` directive labels; panels without a language or tokenizer render exactly as before.
