# component-instance-state Specification

## Purpose

Defines the instance-memo contract behind the `own` utility prop: creation on first render, replay across re-renders (parent-triggered included), per-instance isolation, call-order rules with the misuse warning, unmount lifetime, and SSR/window isolation.

Established by the `component-instance-state` change (2026-09-20).
