# app-edge-caching Specification

## Purpose

Defines the loom app's serving contract: prerendered routes are CDN-served static files with unknown paths falling back to the SPA shell, cache lifetimes match asset mutability (immutable hashed assets, per-deploy HTML), and Contentful publishes trigger rebuilds through a webhook → deploy-hook chain.

Established by the `server-first-loom-app` change (2026-09-15). On-demand per-route regeneration (ISR) is the recorded future evolution.
