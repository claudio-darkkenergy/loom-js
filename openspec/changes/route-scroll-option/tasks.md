...
- [ ] 2.3 Green: thread the option in `route()` — skip `scrollToFragment` for the same-page path and store no `pendingFragment` for the route-changing path when `scroll === false`; for fragmentless route changes (when `scroll !== false`), defer an instant scroll-to-top through the same post-render point as `consumePendingFragment`; `scrollToFragment` and `consumePendingFragment` unchanged
