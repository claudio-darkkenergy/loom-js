...
Keys are the route patterns the app already owns (the same keys as `createRoutes` `config`), so the app wires it with zero translation. A chunk-keyed map (`docs-EP52T6LO.js` → css) was rejected as the app-facing contract: hashed chunk names are unknowable to app code. Internally the plugin derives route ownership per CSS bundle and rolls it up to routes.
