...
- **`timeout?: number`** (`ActivityOptions`, no default): bounds each run — on expiry the run is retired exactly as supersession retires it (signal aborted, future commits dropped, queue/turn released) and it counts as settled even if its promise never resolves, so a hung transform can no longer block a `'serial'`/`'ordered'` queue or pin the settlement count forever. Expiry warns on the always-on console lane, like `maxWait` expiries.
