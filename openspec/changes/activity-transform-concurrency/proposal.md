...
- **Opt-in sequenced mode** (`ActivityOptions`, name settled in design): dispatches queue; a dispatch's transform does not start until the prior dispatch settles, so commits apply in dispatch order and each run's `value` context sees its predecessor's result — the accumulation-pipeline shape.
- Activities without a transform, and synchronous transforms, are unaffected (their commits are already ordered by call order).
- README + activities topic: Transforms and Options sections document both semantics (per the standing propagation requirement).

## Capabilities

### New Capabilities

- `activity-transform-concurrency`: dispatch semantics for async transforms — latest-wins supersession (default), sequenced queuing (opt-in), settlement interaction, and the sync-path invariants both modes preserve.
