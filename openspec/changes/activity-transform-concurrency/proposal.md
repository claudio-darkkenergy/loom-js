...
- The transform context types `input` as `Readonly<I>` (compile-time read-only contract; runtime freezing rejected on record — design D2c), and the docs state the by-reference convention.
