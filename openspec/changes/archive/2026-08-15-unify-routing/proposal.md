...
- **`routing.ts` is deleted** _(revised at apply review, 2026-08-15 — originally a one-release delegating bridge)_: the package is pre-1.0 and the deprecated API's only consumers were core's own tests, README, and server tests, all migrated in this change, so the bridge protected nobody. `router` → `locationEffect`, `onRoute` → `route`, `onRouteUpdate` → `watchLocation` ship as a breaking removal with migrations documented in the changeset. Internal consumers (core specs, server tests, README) migrate to the new names; the README Routing section — which previously documented _only_ the legacy API — is rewritten around `createRoutes`/`route`/`routeEffect` + the new layer-1 API.
...
- **Code:** `packages/core/src/router.ts` (layering, registry, DOM-free capture), `routing.ts` (deleted), `index.ts` (new exports, legacy export removed), `elements/route-link.ts` (unchanged behavior, same `route`), core tests (`routing.spec.ts` deleted — its coverage lives in the new `location.spec.ts` via the replacement names; `lazy-import.spec.ts` + `tests/support/components/router.ts` migrate; new layer-1/isolation specs), server tests migrate off legacy `router`, `packages/core/README.md` Routing section rewrite, `.claude/skills/skill-config.md` routing conventions.
- **Risk:** behavioral parity of the layered activities with the prior subtle semantics — the old activity's `force: true` (fires effects on every update), `didRouteChange`'s origin/pathname/search-only comparison, and `pageRouteEffect`'s same-path skip. Guarded by the existing `route-activation` and migrated `lazy-import` suites plus the new `location.spec` exercising the same behaviors through the replacement names.
- **Release:** `@loom-js/core` minor (pre-1.0, breaking allowed) — additive API (`locationEffect`, `watchLocation`, and the async `renderToString` go-to with the synchronous primitive renamed `renderToStringSync` before its first publish), removal of the legacy `routing.ts` exports, and the module-scope-`createRoutes` SSR fix.
...
5. **`routing.ts` is deleted in this change** _(revised at apply review, 2026-08-15; originally deferred one release behind a delegating bridge)_ — pre-1.0 semver plus zero real external consumers made the migration window pointless.
