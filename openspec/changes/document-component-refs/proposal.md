...
- `packages/core/README.md` Components section gains a **Refs** subsection: `createRef()` → `RefContext`, handing it to a child via the `ref` prop, what the parent gets back (the child's `node()` and life-cycle hooks), `ctxRefs()` iteration in creation order, and when to reach for a ref instead of the component's own `node()`.
- The `props` documentation grows an honest **prop-surface summary**: the reserved props in one compact table with pointers to where each is covered in depth (children/slots/key → element syntax; `routeProps` → routing; `attrs`/`on` → element bindings; `ref` → the new subsection) — no duplicated deep-dives.
