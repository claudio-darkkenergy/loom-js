...
**D1 — Bust compares incoming key to stored key, element-wise — and `cacheKey` is excluded from the signature.** Implementation discovery: `cacheKey` currently participates in `reqSignature` (it lives inside `init`), so a changed key lands in a _different_ cache slot instead of busting the old one — and cycling a key back to a prior value (A→B→A) serves the stale A entry. The mechanism is self-defeating as shipped. Fix: destructure `cacheKey` out before building the signature (`JSON.stringify({ input, init: initWithoutCacheKey })`), then compare the incoming key to the stored key element-wise (`Object.is`) plus a length check. `Object.is` matches the original intent (the stored shape is `any[]`); a JSON compare would false-positive on NaN and function members.
