...
### D2b — Lookup surfaces close with "See also" (added 2026-09-03)

Lookup surfaces — concept hubs like Built-in props, the fragments topic, an element-bindings section — end with a **See also** block linking every place the concept is taught, making the hub a complete index even for connections no single entry's inline pointer carries. Scope is deliberate: teaching topics do _not_ get one — they end with the crafted transitional copy convention, and a mechanical link block under every topic would compete with it and rot into noise (same selective-not-mechanical rule). Maintenance rides the sweep: its per-concept inventory of discussion sites _is_ the See-also source material; the map records each set as part of its cross-link registry, and the standing dead-anchor verification covers rot.

### D3 — The sweep is an artifact with verdicts

`sweep.md` in this change: per topic, candidate concepts found (prose-only names, mechanism-titled headings, split material), each with a verdict — restructure in this change / defer with rationale / fine as-is — maintainer-reviewed before content work, mirroring the map-review gate. For each hub the sweep confirms or creates, its site inventory doubles as the hub's See also set (D2b). Known candidates seeded: **element bindings** (the strongest: the `$` vocabulary — `$click`/`$event`, `$attrs`, `$on`, `$props` — has no heading anywhere _and_ a coverage gap: `$attrs`/`$on`/`$props` behavior is explained nowhere, `$event` surfaces only via Configuration's `appendEvents`, the rest lives in examples — likely a named Components or Element Syntax section, possibly closing the coverage gap in the same stroke); the settlement signal (named across five topics, defined in none — likely deserving a home or a glossary anchor); **transform time** (Element Syntax leans on the phrase throughout — labels resolve, malformed syntax throws, `slot` is consumed "at transform time" — but no section names the template transform as a phase or says when it runs relative to render; the "sugar over the functional form" section describes the mechanism without owning the term); `el()` (element components section vs. value-position usage); reserved props (`document-component-refs` covers — upgraded 2026-09-03 to a full Built-in props section); rich-text/authoring conventions (docs-internal, likely fine).
